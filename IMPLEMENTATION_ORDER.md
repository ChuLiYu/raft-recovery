...
5. `Requeue(job Job)`
6. `MarkDead(jobID)`
7. `GetExpiredJobs(now) []string`
8. `GetJob(jobID) *Job`

#### 2.4 持久化支援

9. `Snapshot() SnapshotData`
10. `Restore(data SnapshotData)`
11. `Validate() error`（驗證不變性）
12. `Stats() map[string]int`
...
5. `EnqueueJobs(jobs []Job) error`
6. `GetStatus() map[string]interface{}`
7. `Stop()`
...
4. `loadConfig()` - YAML + 環境變數 + 旗標

#### 9.3 Main 入口（Day 2）

5. `cmd/queue/main.go` - 呼叫 CLI
